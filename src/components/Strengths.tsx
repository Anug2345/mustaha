...
export const Strengths: React.FC = () => {
  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      case 'FolderTree':
        return <FolderTree className="w-5 h-5 text-[#556447] dark:text-[#AEC89B]" />;
      case 'Eye':
        return <Eye className="w-5 h-5 text-[#556447] dark:text-[#AEC89B]" />;
      case 'MessageSquare':
        return <MessageSquare className="w-5 h-5 text-[#556447] dark:text-[#AEC89B]" />;
      case 'Zap':
        return <Zap className="w-5 h-5 text-[#556447] dark:text-[#AEC89B]" />;
      case 'ShieldCheck':
      default:
        return <ShieldCheck className="w-5 h-5 text-[#556447] dark:text-[#AEC89B]" />;
    }
  };
  ...
  return (
    <section id="strengths" className="py-16 md:py-28 bg-[#F8F7F4] dark:bg-[#111413] relative overflow-hidden">
      {/* Decorative ambient background wave */}
      <div className="absolute top-1/2 left-0 w-96 h-96 bg-[#E8EDE3]/40 dark:bg-[#202723]/30 rounded-full blur-3xl -translate-y-1/2 pointer-events-none -z-0" />
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-[#D3DCCB]/30 dark:bg-[#2E3C32]/20 rounded-full blur-3xl pointer-events-none -z-0" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center p-1 bg-white dark:bg-[#181D1A] border border-[#E2DFD8] dark:border-[#27322A] rounded-xl shadow-2xs max-w-full gap-1">
            {[
              ...
              <button
                ...
                className={`relative px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-semibold rounded-lg transition-colors cursor-pointer min-h-[38px] ${
            ...
            {filteredStrengths.map((strength, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`group relative bg-white dark:bg-[#181D1A] rounded-2xl p-5 sm:p-7 border transition-all duration-300 flex flex-col justify-between overflow-hidden cursor-default ${
                    isExpanded 
                      ? 'border-[#7C8F6A] dark:border-[#8FA67C] shadow-lg ring-2 ring-[#7C8F6A]/20 dark:ring-[#8FA67C]/20' 
                      : 'border-[#E2DFD8] dark:border-[#27322A] shadow-2xs hover:shadow-md hover:border-[#7C8F6A]/60 dark:hover:border-[#8FA67C]/60'
                  } ${isWide ? 'sm:col-span-2 lg:col-span-1' : ''}`}
                >
                  {/* Subtle hover gradient illumination */}
                  <div className="absolute -inset-px bg-gradient-to-br from-[#7C8F6A]/0 via-transparent to-[#7C8F6A]/5 dark:to-[#8FA67C]/5 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none rounded-2xl" />

                  <div className="relative z-10">
                    <div className="flex items-center justify-between mb-5">
                      <motion.div 
                        whileHover={{ rotate: 10, scale: 1.1 }}
                        className="w-12 h-12 rounded-xl bg-[#F4F6F2] dark:bg-[#202723] group-hover:bg-[#E8EDE3] dark:group-hover:bg-[#28352b] flex items-center justify-center transition-colors shadow-2xs"
                      >
                        {getIcon(strength.iconName)}
                      </motion.div>
                      
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-mono font-bold text-[#9CA3AF] dark:text-[#637265] group-hover:text-[#556447] dark:group-hover:text-[#AEC89B] transition-colors">
                          0{strength.id}
                        </span>
                      </div>
                    </div>
                    ...
                    <AnimatePresence>
                      {isExpanded && scenario && (
                        <motion.div
                          ...
                          className="overflow-hidden mt-4 pt-3 border-t border-[#E8EDE3] dark:border-[#27322A]"
                        >
                          <div className="p-3 bg-[#F4F6F2] dark:bg-[#141816] rounded-xl border border-[#D3DCCB]/60 dark:border-[#27322A] text-xs">
                            <p className="font-bold text-[#1F2937] dark:text-[#F3F6F2] flex items-center gap-1.5 mb-1">
                              <CheckCircle2 className="w-3.5 h-3.5 text-[#7C8F6A] dark:text-[#8FA67C]" />
                              Real Operational Scenario:
                            </p>
                            <p className="text-[#4B5563] dark:text-[#A8B3A7] leading-relaxed">
                              {scenario.scenario}
                            </p>
                            <div className="mt-2 text-[10px] font-bold text-[#556447] dark:text-[#AEC89B] uppercase tracking-wider">
                  ...
                  <div className="relative z-10 mt-6 pt-4 border-t border-[#E2DFD8]/60 dark:border-[#27322A] flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-xs text-[#6B7280] dark:text-[#869485]">
                    ...
                    <button
                      ...
                      className="min-h-[36px] inline-flex items-center gap-1 text-xs font-semibold text-[#556447] dark:text-[#AEC89B] hover:text-[#1F2937] dark:hover:text-[#F3F6F2] px-2.5 py-1 rounded-md hover:bg-[#E8EDE3]/60 dark:hover:bg-[#222C26] transition-colors cursor-pointer"
                    >
